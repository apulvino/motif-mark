...
def get_coords(seq):
	'''Takes an input FASTA file and returns, by referencing the motifs list, the coordinate positions per each motif in the FASTA file while also accounting for overlapping motif sequences.'''
	for motif in MOTIF_LIST:
		# initiate a new key into your dict key being the current motif make the value a blank list
		coords_dict[motif] = []
		#print(seq)
		startpos = re.finditer(regex_dict[motif], seq, overlapped=True)
...
legend_y_offset = 30
#adds the names of the genes as headers for each one of the genes drawn in the output image
for motif in MOTIF_LIST:
	...
	ctx.move_to(0, 400)
	ctx.stroke()
	#creates the legend and colors the each of the motifs for said in the list storing all of the information for color based on matplotlib's "cmap" package
for index,motif in enumerate(MOTIF_LIST):
	...
	ctx.stroke()
	legend_y_offset += 10
	#draws length of sequence line and incorporates FASTA headers as labels, also creates boxes representing exons
for line in fasta_seqs:
	...
	motif_counter = 0
	#completes the drawing of the exons
	for index,mark in enumerate(motifmarks):
		ctx.move_to(700, 90)
		ctx.set_line_width(4)
		#ctx.fill
		ctx.stroke()
		#incorporates the motifs based on relative length of sequence/repeat sequence/and motif overlaps
		for a_motif in motifmarks[mark]:
			ctx.set_source_rgba(cmap[index][0], cmap[index][1], cmap[index][2], cmap[index][3])
			ctx.rectangle(a_motif,y-10,len(mark),20)
			...
			#ctx.fill()
			ctx.stroke()
		#sets the colors for each of the motifs 
